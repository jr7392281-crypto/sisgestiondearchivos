...
$buscar = $pdo->prepare("SELECT id_carpeta, carpeta_padre_id FROM tb_carpetas WHERE id_carpeta = :id_carpeta AND id_usuario = :id_usuario LIMIT 1");
$buscar->bindParam(':id_carpeta', $id_carpeta, PDO::PARAM_INT);
$buscar->bindParam(':id_usuario', $id_usuario_sesion, PDO::PARAM_INT);
$buscar->execute();
$subcarpeta = $buscar->fetch(PDO::FETCH_ASSOC);

if (!$subcarpeta) {
    $_SESSION['mensaje'] = "No tienes permiso para eliminar esta subcarpeta.";
...
if ($delete->execute()) {
    $_SESSION['mensaje'] = "Subcarpeta eliminada correctamente.";
    $_SESSION['icono'] = "success";
} else {
    $_SESSION['mensaje'] = "No se pudo eliminar la subcarpeta.";
    $_SESSION['icono'] = "error";
}
